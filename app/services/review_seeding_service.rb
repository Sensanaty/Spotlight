...
class ReviewSeedingService
  ...
  def self.zomato_seed
    puts "\n\e[34mNow seeding randomly generated Zomato reviews\e[0m"
    rand(30..60).times do
      username = "#{Faker::GreekPhilosophers.name}#{Faker::Verb.past_participle}#{Faker::NatoPhoneticAlphabet.code_word}#{rand(1050)}"
  ...
  def self.squarespace_seed
    puts "\n\e[34mNow seeding randomly generated SquareSpace reviews\e[0m"
    rand(20..90).times do
      username = "#{Faker::Verb.past_participle}#{Faker::NatoPhoneticAlphabet.code_word}#{rand(1050)}"
      ZomatoReview.create(reviewer_image: Faker::Avatar.image,
                          reviewer_username: username,
                          reviewer_profile_url: "https://sp0tlight.herokuapp.com",
